...
import TotalRatingHeaderAttraction from '../../sections/attraction/totalRatingHeaderAttraction/TotalRatingHeaderAttraction'
...
const AttractionBox: React.FC<AttractionBoxProps> = ({
  placeData: { shortDescription, content, name, uploadTime, listKeyWords, photos, videos, audios, viewNumber, ratings },
  mark,
  comments,
}) => {
  ...
  return (
    <div>
      <div className='body-attraction'>
        ...
        <div onClick={() => setVisibleQrCode((prevState) => !prevState)} className='qr-open'>
          open QR-CODE
        </div>
        ...
        {ratings && (
          <RatingAttraction isLeaveComment={isLeaveComment} setIsLeaveComment={setIsLeaveComment} ratings={ratings} averageRating={averageRating} />
        )}
